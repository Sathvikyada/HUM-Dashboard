...
// Discord link update email
export async function sendDiscordLinkUpdate(params: {
  to: string;
  name: string;
}): Promise<void> {
  if (!sendgridApiKey) throw new Error('Missing SG_API_KEY');
  if (!emailFrom) throw new Error('Missing EMAIL_FROM environment variable');

  const escapedName = escapeHtml(params.name);
  const discordLink = 'https://discord.gg/YZshAc5GH9';
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="padding:40px 40px 30px;">
              ...
              <p style="margin:0 0 20px;font-size:16px;color:#333;">If you are unable to access the Discord using the link provided to you in the previous email or if the Discord invite expired, please use the following link to join the HackUMass XIII Discord Server:</p>
              <p style="margin:20px 0;text-align:center;">
                <a href="${discordLink}" style="display:inline-block;padding:12px 24px;background-color:#4CAF50;color:#ffffff;text-decoration:none;border-radius:6px;font-weight:600;font-size:16px;">Join Discord Server</a>
              </p>
              <p style="margin:20px 0 0;font-size:14px;color:#666;">
                <a href="${discordLink}" style="color:#4CAF50;word-break:break-all;">${discordLink}</a>
              </p>
              <p style="margin:20px 0 0;font-size:16px;color:#333;">Thank you,<br>HackUMass Team</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:30px 40px;background-color:#f9f9f9;border-top:1px solid #eee;text-align:center;">
              <p style="margin:0;font-size:14px;color:#666;">Questions? Contact us at <a href="mailto:team@hackumass.com" style="color:#4CAF50;">team@hackumass.com</a></p>
            </td>
          </tr>
        </table>
      </td>
...
If you are unable to access the Discord using the link provided to you in the previous email or if the Discord invite expired, please use the following link to join the HackUMass XIII Discord Server:
...
Questions? Contact us at team@hackumass.com`;
  ...
  const msg: any = {
    ...
    bcc: adminEmail,
    replyTo: 'team@hackumass.com',
    subject: 'HackUMass XIII - Updated Discord Link',
    html,
    text,
    trackingSettings: {
      clickTracking: { enable: false },
  ...
  try {
    ...
    if (statusCode >= 200 && statusCode < 300) {
      if (process.env.NODE_ENV !== 'production') {
        console.log(`✅ Discord update email sent to ${params.to}`);
      }
    } else {
      console.error(`⚠️ Unexpected status code: ${statusCode}`);
      throw new Error(`SendGrid returned status ${statusCode}`);
